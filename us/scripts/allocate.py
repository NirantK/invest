...
CATS = {
    ...
    "Oil_Gas": [
        ...
        "XLE",
        "XOP",
    ],  # Oil majors, Canadian, refiners, ETFs
...
def allocate(
    capital: int, top_n: int, min_w: float, max_w: float
    ...
    dvol = ret.where(ret < 0).std() * np.sqrt(252)
    score = mom_avg / dvol
...
def main(capital: int, top_n: int, min_w: float, max_w: float):
    ...
    print(
        f"{'Tick':<5} {'$':>7} {'%':>5} {'12-1':>6} {'6-1':>6} "
        f"{'12w2w':>6} {'Score':>6}  {'Thesis':<25}"
    )
    print("-" * 80)
    ...
    for t in alloc[alloc > 0].sort_values(ascending=False).index:
        ...
        s = scores.loc[t]
        print(
            f"{t:<5} {alloc[t]:>7,.0f} {pct:>4.0f}% {s['12-1']:>5.0f}% "
            f"{s['6-1']:>5.0f}% {s['12w-2w']:>5.0f}% {s['Score']:>6.2f}  "
            f"{THESIS.get(t, ''):<25}"
        )
