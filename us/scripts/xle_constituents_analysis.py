...
def main():
    ...
    for ticker in summary.index:
        ...
        print(
            f"{ticker:<6} {row['XLE_Weight']:>6.2f}%  "
            f"{row['Div_Yield']:>6.2f}%  {row['3M_Mom']:>+8.2f}% "
            f"{row['6M_Mom']:>+8.2f}% {row['Avg_Mom']:>+8.2f}% "
    ...
    header = f"{'Metric':<25} {'XLE ETF':>15} {'Wtd Top 10':>15} {'Difference':>15}"
    print(f"\n{header}")
    print("-" * 70)
    print(
        f"{'3-Month Momentum':<25} {xle_mom_3m:>+14.2f}% "
        f"{weighted_3m:>+14.2f}% {(weighted_3m - xle_mom_3m):>+14.2f}%"
    )
    print(
        f"{'6-Month Momentum':<25} {xle_mom_6m:>+14.2f}% "
        f"{weighted_6m:>+14.2f}% {(weighted_6m - xle_mom_6m):>+14.2f}%"
    )
    print(
        f"{'Average Momentum':<25} {xle_combined:>+14.2f}% "
        f"{weighted_avg_mom:>+14.2f}% "
        f"{(weighted_avg_mom - xle_combined):>+14.2f}%"
    )
    print(
        f"{'Downside Volatility':<25} {xle_down_vol:>14.2f}% "
        f"{weighted_down_vol:>14.2f}% "
        f"{(weighted_down_vol - xle_down_vol):>+14.2f}%"
    )
    print(
        f"{'Combined Score':<25} {xle_score:>14.2f} "
        f"{weighted_score:>14.2f} {(weighted_score - xle_score):>+14.2f}"
    )
    print(
        f"{'Dividend Yield (est)':<25} {'~3.0%':>15} {weighted_div:>14.2f}% {'-':>15}"
    )
    ...
    print("-" * 70)
    print("The analysis shows how XLE's top 10 constituents perform")
    print("individually versus the ETF wrapper. Key insights:")
    print("\n1. Dividend yields vary significantly across constituents")
    print("2. Individual stocks may have better risk-adjusted returns than the ETF")
    print("3. The weighted average approximates XLE but with tracking differences")
    print("4. Top performers by score could be considered for direct investment")
    print(
        "\nConsider: If the best constituents outperform XLE significantly, "
        "direct investment in those stocks may be preferable to the ETF "
        "wrapper."
    )
