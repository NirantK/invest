...
MF_CONSOLIDATED_CACHE = MF_CACHE_DIR.parent / "mf_nav_consolidated.parquet"


def _load_consolidated_cache() -> tuple[np.ndarray, np.ndarray, list[str]] | None:
    """Load consolidated parquet cache if fresh (same day)."""
    import polars as pl
    if not MF_CONSOLIDATED_CACHE.exists():
        return None
    meta_file = MF_CONSOLIDATED_CACHE.with_suffix(".meta")
    if not meta_file.exists():
        return None
    cached_date = meta_file.read_text().strip()
    if cached_date != date.today().isoformat():
        print(f"  [cache] Stale ({cached_date}), refetching...")
        return None
    df = pl.read_parquet(MF_CONSOLIDATED_CACHE)
    dates_arr = np.array(df["date"].to_list())
    fetched = [c for c in df.columns if c != "date"]
    prices = df.select(fetched).to_numpy()
    print(f"  [cache] Loaded consolidated: {len(fetched)} schemes, {len(dates_arr)} days")
    return prices, dates_arr, fetched


def _save_consolidated_cache(
    prices: np.ndarray, dates_arr: np.ndarray, fetched: list[str],
) -> None:
    """Save consolidated parquet cache."""
    import polars as pl
    data = {"date": dates_arr.tolist()}
    for j, name in enumerate(fetched):
        data[name] = prices[:, j].tolist()
    df = pl.DataFrame(data)
    MF_CONSOLIDATED_CACHE.parent.mkdir(parents=True, exist_ok=True)
    df.write_parquet(MF_CONSOLIDATED_CACHE)
    MF_CONSOLIDATED_CACHE.with_suffix(".meta").write_text(date.today().isoformat())
    print(f"  [cache] Saved consolidated: {len(fetched)} schemes")


def fetch_all_mf_numpy(
    scheme_codes: list[int], period: str = "max", max_schemes: int = 1000,
) -> tuple[np.ndarray, np.ndarray, list[str]]:
    ...
    Uses consolidated parquet cache for fast reload (seconds vs minutes).
    Falls back to per-scheme fetch + API if cache is stale.
    Keeps top max_schemes by data coverage (survivorship-bias-aware).
    """
    ...
    # ── Data quality filters (survivorship-bias-aware) ───────────────────
    # Keep dead/merged funds — they were investable during their lifetime.
    # Only reject on data quality grounds:
    #   1. <252 valid days (can't compute 12M momentum)
    #   2. >3 single-day returns exceeding 20% (NAV restatement artifacts)
    candidates = []
    n_bad_data = 0
    n_thin = 0
    for j in range(prices.shape[1]):
        ...
        n_spikes = int((daily_rets > 0.20).sum())
        if n_spikes > 3:
            n_bad_data += 1
            continue  # too many NAV restatement spikes → bad data

        candidates.append((j, n_valid))

    print(f"  [filter] Rejected {n_thin} thin (<252d) + {n_bad_data} bad-data, "
          f"{len(candidates)} pass quality")

    if not candidates:
        return np.array([]), np.array([]), []

    # Rank by valid trading days (more history = more useful for backtest)
    # and keep top max_schemes. Dead funds naturally rank lower but aren't excluded.
    candidates.sort(key=lambda x: x[1], reverse=True)
    keep = [j for j, _ in candidates[:max_schemes]]
    keep.sort()  # restore column order

    if len(candidates) > max_schemes:
        print(f"  [rank] Kept top {max_schemes} by data coverage "
              f"(min {candidates[max_schemes - 1][1]}d, max {candidates[0][1]}d)")

    prices = prices[:, keep]
    fetched = [fetched[j] for j in keep]
    dates_arr = np.array(all_dates)

    _save_consolidated_cache(prices, dates_arr, fetched)
    return prices, dates_arr, fetched
